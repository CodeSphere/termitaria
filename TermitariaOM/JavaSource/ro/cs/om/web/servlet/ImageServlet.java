...
public class ImageServlet extends HttpServlet {
	...
	public Picture getPicture(int pictureId) throws Exception {
		...
		return BLPicture.getInstance().get(pictureId);
	}
	
	public Picture getPicture(String pictureName) throws Exception {
		logger.debug("picture name: " + pictureName);
		return BLPicture.getInstance().getByName(pictureName);
	}

	/**
	 * Dumps Picture's content in an OutputStream
	 */
	private void dumpFile(Picture picture, OutputStream outputstream) throws IOException {
		byte buffer[] = new byte[4096];
		BufferedInputStream bufferedinputstream = new BufferedInputStream(
				new ByteArrayInputStream(picture.getPicture()));
		int i;
