...
public class CalendarShowTimeController extends RootAbstractController{
	
	private static final String VIEW = "Calendar_ShowTime";
	...
	public CalendarShowTimeController() {
		setView(VIEW);
	}
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		if (request.getParameter(END_HOUR) != null) {
			mav.addObject(END_HOUR, ServletRequestUtils.getStringParameter(request, END_HOUR));
		} else {
			mav.addObject(END_HOUR, 23);
		}
		
		if (request.getParameter(END_MINUTES) != null) {
			mav.addObject(END_MINUTES, ServletRequestUtils.getStringParameter(request, END_MINUTES));
		} else {
