...
<c:if test="${!(empty PROJECT)}">
	...
	<table>				
		...
		<security:authorize ifAllGranted="${PERMISSION_CONSTANT.TS_ProjectAdvancedView}">						
			<c:if test="${!(empty PROJECT_DETAILS)}">	
				...
				<tr>
					...
					<td>									
						${PROJECT_DETAILS.notificationPercentage}							
					</td>
				</tr>
				...
				<tr>
					...
					<td>									
						${PROJECT_DETAILS.completenessPercentage}							
					</td>
				</tr>			
				...
				<tr>
					...
					<td>				
						<c:if test="${(PROJECT_DETAILS.observation != '') && !(empty PROJECT_DETAILS.observation) }">
								${PROJECT_DETAILS.observation}
						</c:if>			
					</td>
				</tr>
		...
		<security:authorize ifNotGranted="${PERMISSION_CONSTANT.TS_ProjectAdvancedView}">						
			<c:if test="${!(empty PROJECT_DETAILS) and USER_ID eq PROJECT.managerId}">	
		...
		<tr>
			...
			<td>
				<c:choose>
					<c:when test="${ACTIVITIES eq null}">
						<div class="noFindResults"><spring:message code="activity.no.activities"/></div>
					</c:when>
					<c:otherwise>								            	
						<c:if test="${!(empty ACTIVITIES)}">
							<select id="viewActivities" name="activities" multiple="multiple" size="5" style="width:200px;">
								<c:forEach var="activity" items="${ACTIVITIES}">	
									<option title="${activity.name}">${activity.name}</option>								
								</c:forEach>	
							</select>					
						</c:if>								
					</c:otherwise>
