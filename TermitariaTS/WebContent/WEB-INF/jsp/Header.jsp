...
<script>
	...
	// TS application namespace for YUI widgets
	YAHOO.namespace("ts");
...
// ------------- small loading ------------------------------------------------------------------------------
		...
		// ------------- expired session ------------------------------------------------------------------------
		if (!YAHOO.ts.sessionExpired) {
		    // The Loading panel
		    YAHOO.ts.sessionExpired = new YAHOO.widget.Panel("sessionExpired", { width: "200px", fixedcenter:true,   
		                                              		 draggable: false, zindex:1, modal: true, close: false,
		                                              		 visible: false, underlay: false});
		    YAHOO.ts.sessionExpired.setBody('<spring:message code="session.expired"/> <img border=\"0\" src=\"images/ajaxLoader/ajax-loader-session-expired.gif\"/>');
		    YAHOO.ts.sessionExpired.render(document.body);
		}

// ------------- calendar container ------------------------------------------------------------------------------
		
		if (!YAHOO.ts.calendarContainer) {
	        // The Loading panel
	        ...
	        YAHOO.ts.calendarContainer = new YAHOO.widget.Panel("calendarContainer", {zIndex:200, width: _width, height: _height, x: 0, y: 0, close: false, 
	...
	<div id="page">
    	<div id="top">
        	<div ><img src="<spring:theme code="logo"/>"/></div>
			<div id="topmenu">
