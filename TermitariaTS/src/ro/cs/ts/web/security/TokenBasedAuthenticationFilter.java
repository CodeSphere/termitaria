...
public class TokenBasedAuthenticationFilter extends GenericFilterBean
	...
	private static String LOG_OUT_URL = ConfigParametersProvider.getConfigString("ts.logout.url");
	
	/**Response returned when session is expired*/
	private static String RESPONSE = "<body></body><script>if(typeof(YAHOO) != 'undefined'){YAHOO.ts.sessionExpired.show();setTimeout('window.location=\"" + OM_URL + "\"', 1500);} else {window.location=\"" + OM_URL + "\"}</script>";
...
//				ConcurrentSessionController csc = (ConcurrentSessionController) TSContext.
