...
import ro.cs.cm.ws.client.audit.AuditWebServiceClient;
...
public class AuditThread extends ApplicationObjectSupport implements Runnable {
	...
	public void run() {
		logger.debug("Start sending CM audit event... ");
		try {
			...
			logger.error("Error while sending the CM audit event", exc);
		}
		logger.debug("End sending CM audit event... ");
	}
}
