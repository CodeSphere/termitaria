...
<script>
	...
	// CM application namespace for YUI widgets
	YAHOO.namespace("cm");

	function init() {
		// ------------- small loading ------------------------------------------------------------------------------
		if (!YAHOO.cm.smallLoading) {
	        // The Loading panel
	        YAHOO.cm.smallLoading = new YAHOO.widget.Panel("smallLoading", { width: "35px", x: 0, y: 3, close: false, 
	                                                  		 draggable: false, zindex: 1, modal: true,
	                                                  		 visible: false, underlay: false});

	        YAHOO.cm.smallLoading.setBody("<img src=\"images/ajaxLoader/ajax-loader_snake.gif\"/>");
	        YAHOO.cm.smallLoading.render(document.body);
    	}
		
		// ------------- expired session ------------------------------------------------------------------------
		if (!YAHOO.cm.sessionExpired) {
			// The Loading panel
		    YAHOO.cm.sessionExpired = new YAHOO.widget.Panel("SessionExpiredPanel", { width: "200px", fixedcenter:true,   
		                                              		 draggable: false, zindex:1, modal: true, close: false,
		                                              		 visible: false, underlay: false});
		    YAHOO.cm.sessionExpired.setBody('<spring:message code="session.expired"/> <img border=\"0\" src=\"images/ajaxLoader/ajax-loader-session-expired.gif\"/>');
		    YAHOO.cm.sessionExpired.render(document.body);
		}
