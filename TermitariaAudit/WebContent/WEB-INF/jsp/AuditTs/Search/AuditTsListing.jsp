...
<c:choose>
	<c:when test="${!(empty SEARCH_RESULTS)}">
		<fieldset style="width: 840px;">
			<legend><spring:message code="audit.ts.title.listing" /></legend>
			<form:form id="paginationForm" name="paginationForm" commandName="searchAuditTsBean">
				...
				<table class="list_results" style="width:840px" id="listingTable">
					<tr class="list_results_header">
						...
						<th style="width:200px;">						
							<spring:message code="audit.ts.date" />							
						</th>
			...
			<c:import url="../../Pagination.jsp">
			   <c:param name="currentPage" value="${searchAuditTsBean.currentPage}"/>
			   <c:param name="nbrOfPages" value="${searchAuditTsBean.nbrOfPages}"/>
			   <c:param name="firstPage" value="${searchAuditTsBean.lowerLimit}"/>
			   <c:param name="lastPage" value="${searchAuditTsBean.upperLimit}"/>
			   <c:param name="baseURL" value="AuditTsSearch.htm"/>
			...
			<script>
				<security:authorize ifAllGranted="${PERMISSION_CONSTANT.AUDIT_Delete}">
					YAHOO.util.Event.addListener("deleteAll", "click", submitDeleteWithConfirmation, {url: 'AuditTsSearch.htm?action=DELETE_ALL', formId: "paginationForm",
					container: "RESULTS", confirmationMessage: '<spring:message code="audit.ts.delete.confirmationMessage"/>', selectedForDeletionElementsName: 'auditId',
					noSelectedItemsMessage: '<spring:message code="audit.ts.delete.selectAuditMessage"/>', panelTitle: '<spring:message code="confirm.delete"/>'}, true);
	...
	<c:otherwise>
		<div class="noSearchResults"><spring:message code="audit.ts.no.results.message"/></div>
	</c:otherwise>
</c:choose>
