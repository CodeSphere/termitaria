...
<table id="mainContentTable" border="0" cellpadding="0" cellspacing="0">
	<tr>
	    <td id="submenuCell">
			...
			<a href="#" id="reports"><spring:message code="audit.om.reports" /></a>					
			</div>
...
<span class="section_title"><img src="images/titles/Audit.jpg"><spring:message code="audit.om"/></span>

<table>
<tr>
	<td>
<form:form commandName="searchAuditOmBean" id="searchForm">
...
<table class="tableSearch" >
	...
	<tr>
		...
		<td class="labelTd">
			<spring:message code="audit.om.startDate"/>
		</td>
		
		<td>
			<form:input path="startDate" id="auditSearchForm_startDate" readonly="true" cssClass="formFieldSearch" tabindex="2" onclick="document.getElementById('startCalendar').style.zIndex = 1;instantiateStartEndCalendars('startCalendar','auditSearchForm_startDate', '${TODAY_DATE}', 'startTime', true, 'auditOmStartSearch', document.getElementById('auditSearchForm_endDate').value, false, '1');"/>
		</td>
		<td>
		...
		<td class="labelTd">
			<spring:message code="audit.om.results"/>
		</td>
		<td>
	...
	<tr>
		...
		<td>
			<form:select path="event" id="auditSearchForm_event" cssClass="formFieldSearch" tabindex="6">
				<form:option value="-1"><spring:message code="audit.om.choose.event" /></form:option>
				<c:forEach var="event" items="${AUDIT_EVENTS_OM}">
		...
		<c:if test="${IS_ADMIN_IT == true}">	
			...
			<td class="labelTd">
				<spring:message code="audit.om.organisation"/>
			</td>
			<td>
				<form:select path="organisationId" cssClass="formFieldSearch" onchange="instantiatePersonAutoCompleteForOrganisation(this.options[selectedIndex].value, 'auditSearchForm_user', 'userAutoCompleteContainer', 'personId')" tabindex="7">
					<form:option value="-1"><spring:message code="audit.om.choose.organisation" /></form:option>
	...
	<tr>
		<td colspan="5"></td>
		<td>			
			<input type="button" class="button" id="searchAuditOm" value="<spring:message code="search"/>" tabindex="${searchButtonTabIndex}"/>			
		</td>				
	</tr>
</table>
...
<script type="text/javascript">
	...
	// -------------------------------REPORTS MENU BUTTON-------------------------------------------------
	var	getObject = new ObjSubmit("AuditOmReports.htm", "", "MAIN_CONTENT");
	...
	// -------------------------------SUBMIT SEARCH FORM---------------------------------------------------
	submitForm('AuditOmSearch.htm', 'searchForm', 'RESULTS');

	// ------------------------------- SEARCH AUDIT OM ----------------------------------------------------
	YAHOO.util.Event.addListener("searchAuditOm", "click", validateAndSubmitAuditSearchForm, { url : "AuditOmSearch.htm", formId : "searchForm", container : "RESULTS", userHiddenFormElementId: "personId", userInputAutoCompleteId: "auditSearchForm_user"}
				, true);
