...
<c:choose>
	<c:when test="${!(empty SEARCH_RESULTS)}">
		<fieldset style="width: 840px;">
			<legend><spring:message code="audit.dm.title.listing" /></legend>
			<form:form id="paginationForm" name="paginationForm" commandName="searchAuditDmBean">
				...
				<table class="list_results" style="width:840px" id="listingTable">
						...
						<th style="width:200px;">						
							<spring:message code="audit.dm.date" />							
						</th>
			...
			<c:import url="../../Pagination.jsp">
			   <c:param name="currentPage" value="${searchAuditDmBean.currentPage}"/>
			   <c:param name="nbrOfPages" value="${searchAuditDmBean.nbrOfPages}"/>
			   <c:param name="firstPage" value="${searchAuditDmBean.lowerLimit}"/>
			   <c:param name="lastPage" value="${searchAuditDmBean.upperLimit}"/>
			   <c:param name="baseURL" value="AuditDmSearch.htm"/>
			...
			<script>
				<security:authorize ifAllGranted="${PERMISSION_CONSTANT.AUDIT_Delete}">
					YAHOO.util.Event.addListener("deleteAll", "click", submitDeleteWithConfirmation, {url: 'AuditDmSearch.htm?action=DELETE_ALL', formId: "paginationForm",
	...
	<c:otherwise>
		<div class="noSearchResults"><spring:message code="audit.dm.no.results.message"/></div>
	</c:otherwise>
</c:choose>
