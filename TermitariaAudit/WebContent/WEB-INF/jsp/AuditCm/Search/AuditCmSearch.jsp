...
<table id="mainContentTable" border="0" cellpadding="0" cellspacing="0">
	<tr>
	    <td id="submenuCell">
			...
			<a href="#" id="reports"><spring:message code="audit.cm.reports" /></a>					
			</div>
...
<span class="section_title"><img src="images/titles/Audit.jpg"><spring:message code="audit.cm"/></span>

<table>
<tr>
	<td>
<form:form commandName="searchAuditCmBean" id="searchForm">
...
<table class="tableSearch" >
	...
	<tr>
		<td class="labelTd">
			<spring:message code="audit.cm.user" />
		</td>
		<td>
		...
		<td class="labelTd">
			<spring:message code="audit.cm.startDate"/>
		</td>
		<td>
			<form:input path="startDate" id="auditSearchForm_startDate" readonly="true" cssClass="formFieldSearch" tabindex="2" onclick="document.getElementById('startCalendar').style.zIndex = 1;instantiateStartEndCalendars('startCalendar','auditSearchForm_startDate', '${TODAY_DATE}', 'startTime', true, 'auditCmStartSearch', document.getElementById('auditSearchForm_endDate').value, false, '1');"/>
		</td>
		<td>
		...
		<td class="labelTd">
			<spring:message code="audit.cm.results"/>
		</td>
		<td>
	...
	<tr>	
		<td class="labelTd">
			<spring:message code="audit.cm.message"/>
		</td>
		<td>
			<form:input path="message" id="auditSearchForm_message"  cssClass="formFieldSearch" tabindex="4"/>
		</td>
		
		<td class="labelTd">
			<spring:message code="audit.cm.endDate"/>
		</td>
		<td>
			<form:input path="endDate" id="auditSearchForm_endDate" readonly="true" cssClass="formFieldSearch" tabindex="5" onclick="document.getElementById('endCalendar').style.zIndex = 1;instantiateStartEndCalendars('endCalendar','auditSearchForm_endDate', '${TODAY_DATE}', 'endTime', false, 'auditCmEndSearch', document.getElementById('auditSearchForm_startDate').value, false, '1');"/>
		</td>
		<td>
	...
	<tr>
		<td class="labelTd">
			<spring:message code="audit.cm.event" />
		</td>
		<td>
			<form:select path="event" id="auditSearchForm_event" cssClass="formFieldSearch" tabindex="6">
				<form:option value="-1"><spring:message code="audit.cm.choose.event" /></form:option>
				<c:forEach var="event" items="${AUDIT_EVENTS_CM}">
		...
		<c:if test="${IS_ADMIN_IT == true}">	
			...
			<td class="labelTd">
				<spring:message code="audit.cm.organisation"/>
			</td>
			<td>
				<form:select path="organisationId" cssClass="formFieldSearch" onchange="instantiatePersonAutoCompleteForOrganisation(this.options[selectedIndex].value, 'auditSearchForm_user', 'userAutoCompleteContainer', 'personId')" tabindex="7">
					<form:option value="-1"><spring:message code="audit.cm.choose.organisation" /></form:option>
	...
	<tr>
		<td colspan="5"></td>
		<td>			
			<input type="button" class="button" id="searchAuditCm" value="<spring:message code="search"/>" tabindex="${searchButtonTabIndex}"/>			
		</td>				
	</tr>
</table>
...
<script type="text/javascript">
	...
	// -------------------------------REPORTS MENU BUTTON-------------------------------------------------
	var	getObject = new ObjSubmit("AuditCmReports.htm", "", "MAIN_CONTENT");
	YAHOO.util.Event.addListener("reports", "click", getContentFromUrl, getObject, true);

	// -------------------------------HOME MENU BUTTON----------------------------------------------------
	var getObject = new ObjSubmit("AuditOmSearch.htm", "", "MAIN_CONTENT");
	YAHOO.util.Event.addListener("search", "click", getContentFromUrl, getObject, true);

	// -------------------------------SUBMIT SEARCH FORM---------------------------------------------------
	submitForm('AuditCmSearch.htm', 'searchForm', 'RESULTS');

	// ------------------------------- SEARCH AUDIT CM ----------------------------------------------------
	YAHOO.util.Event.addListener("searchAuditCm", "click", validateAndSubmitAuditSearchForm, { url : "AuditCmSearch.htm", formId : "searchForm", container : "RESULTS", userHiddenFormElementId: "personId", userInputAutoCompleteId: "auditSearchForm_user"}
				, true);
