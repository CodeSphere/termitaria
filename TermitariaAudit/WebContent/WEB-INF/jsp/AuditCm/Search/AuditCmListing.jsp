...
<c:choose>
	<c:when test="${!(empty SEARCH_RESULTS)}">
		<fieldset style="width: 840px;">
			<legend><spring:message code="audit.cm.title.listing" /></legend>
			<form:form id="paginationForm" name="paginationForm" commandName="searchAuditCmBean">
				...
				<table class="list_results" style="width:840px" id="listingTable">
					<tr class="list_results_header">
						...
						<th style="width:200px;">						
							<spring:message code="audit.cm.date" />							
						</th>
			...
			<c:import url="../../Pagination.jsp">
			   <c:param name="currentPage" value="${searchAuditCmBean.currentPage}"/>
			   <c:param name="nbrOfPages" value="${searchAuditCmBean.nbrOfPages}"/>
			   <c:param name="firstPage" value="${searchAuditCmBean.lowerLimit}"/>
			   <c:param name="lastPage" value="${searchAuditCmBean.upperLimit}"/>
			   <c:param name="baseURL" value="AuditCmSearch.htm"/>
			...
			<script>
				<security:authorize ifAllGranted="${PERMISSION_CONSTANT.AUDIT_Delete}">
					YAHOO.util.Event.addListener("deleteAll", "click", submitDeleteWithConfirmation, {url: 'AuditCmSearch.htm?action=DELETE_ALL', formId: "paginationForm",
					container: "RESULTS", confirmationMessage: '<spring:message code="audit.cm.delete.confirmationMessage"/>', selectedForDeletionElementsName: 'auditId',
					noSelectedItemsMessage: '<spring:message code="audit.cm.delete.selectAuditMessage"/>', panelTitle: '<spring:message code="confirm.delete"/>'}, true);
	...
	<c:otherwise>
		<div class="noSearchResults"><spring:message code="audit.cm.no.results.message"/></div>
	</c:otherwise>
</c:choose>
