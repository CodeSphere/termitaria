...
import ro.cs.logaudit.entity.AuditDmReportCriteriaBean;
 ...
 *
 */
public class AuditDmReportsController extends RootSimpleFormController{

	private static final String FORM_VIEW = "AuditDmReports";
	...
	private static final String AUDIT_DM_REPORTS_CRITERIA_BEAN = "auditDmReportCriteriaBean";
	private static final String CREATE_ERROR = "audit.dm.report.create.error";
	...
	private static final String AUDIT_OM_REPORTS_SHOW_CRITERIA = "audit.dm.report.show.criteria";
	...
	public AuditDmReportsController(){
		setFormView(FORM_VIEW);
		setSuccessView(SUCCESS_VIEW);
		setCommandName(AUDIT_DM_REPORTS_CRITERIA_BEAN);
		setCommandClass(AuditDmReportCriteriaBean.class);
	}
	...
	protected ModelAndView onSubmit(HttpServletRequest request,	HttpServletResponse response, Object command, BindException errors)
		...
		try {
			AuditDmReportCriteriaBean reportRequestParamBean = (AuditDmReportCriteriaBean) command;
			...
			String reportServletRequest = REPORT_SERVLET_URL_VALUE.concat("?").concat(IReportsWsClientConstant.AUDIT_EVENTS_REPORT_START_DATE_PARAM).concat("=").concat(startDate)
											...
											.concat("&").concat(IReportsWsClientConstant.AUDIT_EVENTS_REPORT_MODULE_ID_PARAM).concat("=").concat(String.valueOf(IConstant.NOM_MODULE_DM_LABEL_KEY))
			...
			logger.error("", exc);
			errorMessages.add(messageSource.getMessage(CREATE_ERROR, new Object[] {ICodeException.AUDITDM_REPORT_CREATE, ControllerUtils.getInstance().getFormattedCurrentTime()}
	...
	protected Map referenceData( HttpServletRequest request) throws Exception {
		...
		try {
			...
			if (!userAuth.isAdminIT()){
				map.put(JSON_PERSONS, ControllerUtils.getInstance().getPersonsFromOrgAsJSON((ControllerUtils.getInstance().getOrganisationIdFromSession(request)), RequestContextUtils.getLocale(request), errorMessages, messageSource));
			}
		} catch (Exception exc) {
