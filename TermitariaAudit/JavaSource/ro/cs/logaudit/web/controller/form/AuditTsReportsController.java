...
import ro.cs.logaudit.entity.AuditTsReportCriteriaBean;
...
public class AuditTsReportsController extends RootSimpleFormController {

	private static final String FORM_VIEW = "AuditTsReports";
	...
	private static final String AUDIT_TS_REPORTS_CRITERIA_BEAN = "auditTsReportCriteriaBean";
	private static final String CREATE_ERROR = "audit.ts.report.create.error";
	...
	private static final String AUDIT_TS_REPORTS_SHOW_CRITERIA = "audit.ts.report.show.criteria";
	...
	public AuditTsReportsController(){
		setFormView(FORM_VIEW);
		setSuccessView(SUCCESS_VIEW);
		setCommandName(AUDIT_TS_REPORTS_CRITERIA_BEAN);
		setCommandClass(AuditTsReportCriteriaBean.class);
	}
	...
	protected ModelAndView onSubmit(HttpServletRequest request,	HttpServletResponse response, Object command, BindException errors)
		...
		try {
			AuditTsReportCriteriaBean reportRequestParamBean = (AuditTsReportCriteriaBean) command;
			SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy HH:mm");
			...
			String reportServletRequest = REPORT_SERVLET_URL_VALUE.concat("?").concat(IReportsWsClientConstant.AUDIT_EVENTS_REPORT_START_DATE_PARAM).concat("=").concat(startDate)
											...
											.concat("&").concat(IReportsWsClientConstant.AUDIT_EVENTS_REPORT_MODULE_ID_PARAM).concat("=").concat(String.valueOf(IConstant.NOM_MODULE_TS_LABEL_KEY))
			...
			logger.error("", exc);
			errorMessages.add(messageSource.getMessage(CREATE_ERROR, new Object[] {ICodeException.AUDITTS_REPORT_CREATE, ControllerUtils.getInstance().getFormattedCurrentTime()}
	...
	protected Map referenceData( HttpServletRequest request) throws Exception {
		...
		try{
			...
			//add all the TS audit event types
			...
			errorMessages.add(messageSource.getMessage(AUDIT_TS_REPORTS_SHOW_CRITERIA, new Object[] {}, RequestContextUtils.getLocale(request)));
		}
