...
import ro.cs.logaudit.entity.AuditCmReportCriteriaBean;
 ...
 *
 */
public class AuditCmReportsController extends RootSimpleFormController {

	private static final String FORM_VIEW = "AuditCmReports";
	...
	private static final String AUDIT_CM_REPORTS_CRITERIA_BEAN = "auditCmReportCriteriaBean";
	private static final String CREATE_ERROR = "audit.cm.report.create.error";
	...
	private static final String AUDIT_CM_REPORTS_SHOW_CRITERIA = "audit.cm.report.show.criteria";
	...
	public AuditCmReportsController(){
		setFormView(FORM_VIEW);
		setSuccessView(SUCCESS_VIEW);
		setCommandName(AUDIT_CM_REPORTS_CRITERIA_BEAN);
		setCommandClass(AuditCmReportCriteriaBean.class);
	}
	...
	protected ModelAndView onSubmit(HttpServletRequest request,	HttpServletResponse response, Object command, BindException errors)
		...
		try {
			AuditCmReportCriteriaBean reportRequestParamBean = (AuditCmReportCriteriaBean) command;
			SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy HH:mm");
			...
			String reportServletRequest = REPORT_SERVLET_URL_VALUE.concat("?").concat(IReportsWsClientConstant.AUDIT_EVENTS_REPORT_START_DATE_PARAM).concat("=").concat(startDate)
											...
											.concat("&").concat(IReportsWsClientConstant.AUDIT_EVENTS_REPORT_MODULE_ID_PARAM).concat("=").concat(String.valueOf(IConstant.NOM_MODULE_CM_LABEL_KEY))
			...
			logger.error("", exc);
			errorMessages.add(messageSource.getMessage(CREATE_ERROR, new Object[] {ICodeException.AUDITCM_REPORT_CREATE, ControllerUtils.getInstance().getFormattedCurrentTime()}
	...
	protected Map referenceData( HttpServletRequest request) throws Exception {
		...
		try{
			...
			//add all the CM audit event types
			...
			errorMessages.add(messageSource.getMessage(AUDIT_CM_REPORTS_SHOW_CRITERIA, new Object[] {}, RequestContextUtils.getLocale(request)));
		}
