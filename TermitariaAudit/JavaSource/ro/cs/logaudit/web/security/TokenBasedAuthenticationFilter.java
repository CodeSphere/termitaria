...
public class TokenBasedAuthenticationFilter extends GenericFilterBean
	...
	private static String OM_URL = ConfigParametersProvider.getConfigString("om.url");
	private static String LOG_OUT_URL = ConfigParametersProvider.getConfigString("audit.logout.url");
	
	/**Response returned when session is expired*/
	private static String RESPONSE = "<body></body><script>if(typeof(YAHOO) != 'undefined'){YAHOO.audit.sessionExpired.show();setTimeout('window.location=\"" + OM_URL + "\"', 1500);} else {window.location=\"" + OM_URL + "\"}</script>";
	private static String RESPONSE_SIGN_OUT = "<body></body><script>window.location=\"" + OM_URL + "\"</script>";
	
	
	private String HAS_OM_MODULE = "HAS_OM_MODULE";
	private String HAS_DM_MODULE = "HAS_DM_MODULE";
	private String HAS_CM_MODULE = "HAS_CM_MODULE";
	private String HAS_TS_MODULE = "HAS_TS_MODULE";
	...
	/**
	 * @author tekin
	 * 12.03.2012 - Commented the ConcurrentSessionController functionality.
	 * @param request
	 * @param response
	 * @param chain
	 * @throws IOException
	...
	public void doFilter(ServletRequest req,
		...
		if (securityToken != null) {
			...
			try {
				...
				UserAuth userAuth = BLUser.getInstance().getBySecurityToken(securityToken);
				
				//verify which modules the user's organisation has
				boolean hasOMModule = false;
				boolean hasDMModule = false;
				boolean hasCMModule = false;
				boolean hasTSModule = false;
				for(Integer moduleId : userAuth.getOrganisationModulesIds()){
					if (moduleId.equals(IConstant.NOM_MODULE_OM_LABEL_KEY)){
						hasOMModule = true;
					}
					if (moduleId.equals(IConstant.NOM_MODULE_DM_LABEL_KEY)){
						hasDMModule = true;
					}
					if (moduleId.equals(IConstant.NOM_MODULE_CM_LABEL_KEY)){
						hasCMModule = true;
					}
					if (moduleId.equals(IConstant.NOM_MODULE_TS_LABEL_KEY)){
						hasTSModule = true;
					}
				}
				
				request.getSession().setAttribute(HAS_OM_MODULE, hasOMModule);
				request.getSession().setAttribute(HAS_DM_MODULE, hasDMModule);
				request.getSession().setAttribute(HAS_CM_MODULE, hasCMModule);
				request.getSession().setAttribute(HAS_TS_MODULE, hasTSModule);
				
				authResult = new UsernamePasswordAuthenticationToken(userAuth, userAuth.getPassword(),
						userAuth.getAuthorities());
				
				//Checking if a user didn't overcome the maximum number of allowed sessions
				//-------------------------------------------------------------------------------------
				// Because 'authenticationManager.authenticate(authRequest)' was checking also for
				// the maximum number of sessions/user, we are now doing it manually:
				// - setting the Details (from here the controller will extract the sessionId)
				((UsernamePasswordAuthenticationToken) authResult).
					setDetails(new SessionIdentifierAwareImpl(request.getSession().getId()));
				// - obtaining ConcurrentSessionController bean from Spring Application Context
				
//				ConcurrentSessionController csc = (ConcurrentSessionController) AuditContext.
...
//				csc.checkAuthenticationAllowed(authResult);
				...
				if (ignoreFailure) {
					...
					authenticationEntryPoint
							.commence(request, response, 
									new AuthenticationServiceException(failed.getMessage(), failed));
				}
		...
		if(securityToken == null && SecurityContextHolder.getContext().getAuthentication() == null){	
...
//	public int getOrder() {
//		return SecurityFilters.BASIC_AUTH_FILTER;
//	}
...
class SessionIdentifierAwareImpl implements SessionIdentifierAware {
	...
	/* (non-Javadoc)
	 * @see org.springframework.security.concurrent.SessionIdentifierAware#getSessionId()
	 */
